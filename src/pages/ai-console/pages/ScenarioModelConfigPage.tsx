      ...
      <div className="rounded-[24px] border border-[var(--color-border)] bg-[linear-gradient(180deg,#FFFFFF_0%,#FAFBFC_100%)] p-6">
        ...
        <div className="mt-4 flex items-center gap-2 flex-wrap">
          {([
            ...
            <Button key={key} variant={!showTopology && activeTab === key ? 'primary' : 'secondary'} size="sm"
              onClick={() => { setShowTopology(false); onTabChange(key); }}>
              <Icon size={14} />{label}
            </Button>
          ))}
          <Button variant={showTopology ? 'primary' : 'secondary'} size="sm" onClick={() => setShowTopology(p => !p)}>
            <Workflow size={14} />流水线拓扑
          </Button>
          {!showTopology && (
            <Button variant={compareMode ? 'primary' : 'secondary'} size="sm"
      ...
      {showTopology && (
        <SectionCard title="AI 能力流水线拓扑">
              ...
              {topologyNodes.map((node, i, arr) => (
                <div key={node.nodeId} className="flex items-start flex-shrink-0">
                  <div className={`rounded-[18px] border-2 px-4 py-3 min-w-[100px] text-center ${node.enabled ? (node.effectiveSource === 'node' ? 'border-[var(--color-warning)] bg-[rgba(234,179,8,0.06)]' : 'border-[var(--color-success)] bg-[rgba(5,150,105,0.04)]') : 'border-[var(--color-border-light)] bg-[rgba(15,23,42,0.02)] opacity-60'}`}>
                    ...
                    <div className="mt-1.5"><Badge variant={node.enabled ? (node.effectiveSource === 'node' ? 'yellow' : 'green') : 'gray'}>{node.enabled ? (node.effectiveSource === 'node' ? '节点覆盖' : '场景继承') : '已停用'}</Badge></div>
                    <div className="text-[11px] text-[var(--color-text-light)] mt-1.5">{node.effectiveModel}</div>
                    {node.humanConfirmationRequired && <div className="text-[11px] text-[var(--color-warning)] mt-0.5">需人工确认</div>}
                  </div>
                  {i < arr.length - 1 && (
          ...
          <div className="mt-2 flex gap-3 text-xs text-[var(--color-text-secondary)]">
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-[rgba(5,150,105,0.3)] border border-[var(--color-success)]" />场景继承</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-[rgba(234,179,8,0.2)] border border-[var(--color-warning)]" />节点覆盖</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-sm bg-[rgba(15,23,42,0.03)] border border-[var(--color-border-light)]" />已停用</span>
          </div>
        </SectionCard>
      )}
            ...
            {effectiveScenarioPolicies.map(item => (
              <button key={item.scenarioConfigId} type="button"
                ...
                <div className="flex gap-1 mt-1.5">
                  <Badge variant={item.riskTone}>{item.manualReviewRequired ? '强制复核' : item.humanSendAllowed ? '可发送' : '仅建议'}</Badge>
                </div>
              </button>
            ))}
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs border-collapse">
                <thead><tr className="border-b border-[var(--color-border)]">
                  ...
                  {compareIds.map(id => { const p = effectiveScenarioPolicies.find(pp => pp.scenarioConfigId === id); return <th key={id} className="text-left px-3 py-2 font-semibold">{p ? displayScenario(p.scenario) : id}</th>; })}
                </tr></thead>
                <tbody>{compareRows(effectiveScenarioPolicies, compareIds).map(row => (
                  <tr key={row.label} className="border-b border-[var(--color-border-light)]">
                    ...
                    <td className="px-3 py-2">{row.left}</td><td className="px-3 py-2">{row.right}</td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
          ) : <div className="text-xs text-[var(--color-text-secondary)]">选择 2 个场景卡片开始对比。</div>}
        </SectionCard>
      )}

      {/* Scenario Tab */}
      {activeTab === 'scenario' && activeScenarioDraft && (
        <div className="grid grid-cols-[300px_minmax(0,1fr)] gap-4 max-[1200px]:grid-cols-1">
          <SectionCard title="场景列表">
            <div className="space-y-1.5">
              {effectiveScenarioPolicies.map(item => (
                <button key={item.scenarioConfigId} type="button"
                  className={`w-full rounded-[14px] border p-3 text-left transition-colors ${selectedScenarioId === item.scenarioConfigId ? 'border-[var(--color-primary)] bg-[var(--color-primary-bg)]' : 'border-[var(--color-border-light)] hover:border-[var(--color-border)]'}`}
                  ...
                  <div className="flex gap-1 flex-wrap mt-2">
                    <Badge variant={item.aiSuggestAllowed ? 'blue' : 'gray'}>{item.aiSuggestAllowed ? 'AI 建议' : '建议关闭'}</Badge>
                    {item.activeNodeOverrideCount > 0 && <Badge variant="yellow">{item.activeNodeOverrideCount} 个节点覆盖</Badge>}
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="space-y-4">
            {/* Action bar */}
            <div className="flex items-center justify-between gap-3 flex-wrap rounded-[18px] border border-[var(--color-border)] bg-white px-4 py-2.5">
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" disabled={!scenarioDirty} onClick={() => { void onUpdateScenarioModelConfig(activeScenarioDraft); setScenarioDirty(false); }}>保存</Button>
              </div>
            </div>
            ...
            <SectionCard title="治理与安全">
              <div className="grid grid-cols-3 gap-2 mb-3 max-[1000px]:grid-cols-1">
          ...
          <SectionCard title="节点列表">
            <div className="space-y-1.5">
              {effectiveNodePolicies.map(item => (
                <button key={item.nodeConfigId} type="button"
                  className={`w-full rounded-[14px] border p-3 text-left transition-colors ${selectedNodeId === item.nodeConfigId ? 'border-[var(--color-primary)] bg-[var(--color-primary-bg)]' : 'border-[var(--color-border-light)] hover:border-[var(--color-border)]'}`}
                  ...
                  <div className="flex gap-1 flex-wrap mt-2">
                    <Badge variant={item.effectiveSource === 'scenario' ? 'blue' : 'yellow'}>{item.effectiveSource === 'scenario' ? '场景继承' : '节点覆盖'}</Badge>
                    <Badge variant={item.humanConfirmationRequired ? 'red' : 'green'}>{item.humanConfirmationRequired ? '需确认' : '自动'}</Badge>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between gap-3 flex-wrap rounded-[18px] border border-[var(--color-border)] bg-white px-4 py-2.5">
              ...
              <div className="flex gap-2">
                ...
                <Button size="sm" disabled={!nodeDirty} onClick={() => { void onUpdatePipelineNodeConfig(activeNodeDraft); setNodeDirty(false); }}>保存</Button>
              </div>
            </div>

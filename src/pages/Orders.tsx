...
export function OrdersPage({ result, customers, query, onQueryChange, selectedOrderId, onSelectOrder }: OrdersPageProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <PanelCard title="订单列表" description="按支付、履约、国家和风险统一浏览订单，点击订单号或查看按钮后在抽屉查看详情。" className="overflow-hidden">
                  ...
                  <tbody>
                    {result.items.map(order => {
                      ...
                      return (
                        <tr
                          key={order.id}
                          className={`cursor-pointer border-b border-[var(--color-border-light)] ${selectedOrder?.id === order.id ? 'bg-[var(--color-primary-bg)]' : 'hover:bg-[rgba(255,255,255,0.42)]'}`}
                          onClick={() => {
                        ...
                        >
                          <td className="px-4 py-3 text-[13px]">
                            ...
                            <div className="text-[11px] text-[var(--color-text-light)] mt-1">${order.total.toFixed(2)}</div>
                          </td>
      ...
      <Drawer open={drawerOpen && Boolean(selectedOrder)} onClose={() => setDrawerOpen(false)} title={selectedOrder?.id ?? '订单详情'}>
        {selectedOrder ? (
          ...
          <EmptyState title="尚未选择订单" description="从列表选择订单后查看详情。" compact />
        )}
      </Drawer>
    </div>
  );
